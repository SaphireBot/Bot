...
export default async (message, correctAnswer, Quiz, question) => {
    ...
    async function collect(int) {
        ...
        if (alreadyReplied.includes(user.id))
            return await int.reply({
                content: `${e.Animated.SaphireSleeping} | Você já errou esta pergunta, ok? Espere pela próxima, é rapidinho.`,
                ephemeral: true
            })
        ...
        const userTag = await client.users.fetch(question.suggestedBy).then(user => `${user.username} \`${user.id}\``).catch(() => "")
    ...
    async function end(reason) {
        ...
        if (reason == 'messageDelete') {
            ...
            Quiz.unregister()
            return Quiz.channelSend({
                content: `${e.Animated.SaphirePanic} | APAGARAAAAAM a mensagem. OMG!\n${e.Check} | Mas ta de boa, eu salvei os pontos, beleza?`
            })
        }
