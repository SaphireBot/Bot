...
export default async (interaction, commandData) => {
    ...
    if (!execute)
        return interaction.reply({
            content: `${e.Animated.SaphirePanic} | Sub-Função não encontrada. #168680000`,
            ephemeral: true
        })
    ...
    async function enable() {
        if (TempCallManager.guildsId.includes(guildId))
            return interaction.update({
                content: `${e.Animated.SaphireReading} | Eu olhei aqui e este servidor já tem o Tempo em Call ativado.`,
                components: []
            }).catch(() => { })
    ...
    async function disable() {
        ...
        if (!TempCallManager.guildsId.includes(guildId))
            return interaction.update({
                content: `${e.Animated.SaphireReading} | Meus sistemas dizem que este servidor não tem o Tempo em Call ativo.`,
                components: []
            }).catch(() => { })
    ...
    async function muteTime() {
        ...
        if (!TempCallManager.guildsId.includes(guildId))
            return interaction.update({
                content: `${e.Animated.SaphireReading} | O sistema precisa estar ativo para ativar este recurso.`,
                components: []
            }).catch(() => { })
