...
export default async interaction => {
    ...
    if (
        ...
        return interaction.reply({
            content: `${e.DenyX} | Apenas administradores e o autor da mensagem pode entrar aqui, ok?`,
            ephemeral: true,
        })
    ...
    return await Database.Guild.findOneAndUpdate(
        ...
        .catch(err => interaction.editReply({
            content: `${e.Animated.SaphirePanic} | Não foi possível desativar o Caps Lock Filter.\n${e.bug} | \`${err}\``
        }).catch(() => { }))
