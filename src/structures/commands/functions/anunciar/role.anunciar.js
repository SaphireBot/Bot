...
export default async (interaction, guildResources) => {
    ...
    if (member.roles.cache.has(roleId))
        return member.roles.remove(roleId, `${member.user.username} não quer ser notificado do jornal do servidor.`)
    ...
    return member.roles.add(roleId, `${member.user.username} quer ser notificado do jornal do servidor.`)
        .then(async () => await interaction.reply({
