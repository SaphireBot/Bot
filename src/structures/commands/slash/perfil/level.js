...
import viewWallpapers from './level/wallpapers.level.js';
...
export default {
    ...
    options: [
        ...
        {
            name: 'view_wallpaper',
            description: 'Ver todos os wallpapers disponíveis',
            type: 3,
            autocomplete: true
        },
        {
    ...
    async execute({ interaction, clientData, e }) {
        ...
        if (options.getString('view_wallpaper')) return viewWallpapers(interaction)
        ...
        if (!userData)
            return await interaction.reply({
                content: `${e.Database} | DATABASE | O usuário **${user?.username || 'Indefinido'}** *\`${user?.id || '0'}\`* não foi encontrado no meu banco de dados.`,
                ephemeral: true
            })
        ...
        async function bgAcess() {
            ...
            const format = bgacess.map(id => `${client.users.resolve(id)?.username || 'Não encontrado'} - \`${id}\``).join('\n')
