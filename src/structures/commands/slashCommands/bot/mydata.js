    ...
    async execute({ interaction, guild }, commandData) {
        ...
        const dataObjectFormat = JSON.stringify({
            ...
            Perfil: {
                ...
                Family: (data.Perfil?.Family?.length || 0) + ' usuários - Confira em /perfil',
                Parcas: (data.Perfil?.Parcas?.length || 0) + ' usuários - Confira em /perfil',
                Marry: {
                    ...
                    if (value.deletedCount == 1)
                        return interaction.update({
                            content: `${e.CheckV} | ${commandData.userId == interaction.user.id
                                ...
                                : `Os dados de **${(await client.users.fetch(commandData.userId).then(user => user.tag).catch(() => 'Not Found'))} - \`${commandData.userId}\`** foram deletados com sucesso.`
                                }`,
                            files: [],
                            components: []
                        }).catch(() => { })
