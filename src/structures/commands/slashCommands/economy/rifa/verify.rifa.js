...
export default async interaction => {
    ...
    const LastWinner = TempPrize > 0
        ...
        : `${client.users.resolve(rifaData.LastWinner)?.tag || 'Ninguém ganhou por enquanto'} - \`${rifaData.LastWinner || 0}\``

    const numbersArray = [...Array(91).keys()].slice(1)
    const availableNumbers = numbersArray
        .map(num => {
            return numbers.find(data => data.number === num)
                ? null
                : `${num < 10 ? `0${num}` : num}`
        })
        .filter(i => i)
    ...
    const replyData = {
        embeds: [{
            ...
            fields: [
                {
                    name: '🏷️ Rifa Atual',
                    value: `${numbers.length} foram compradas somando ${(numbers.length * 1000) + TempPrize} ${moeda}`
                },
                {
                    name: `🧩 Última Rifa`,
