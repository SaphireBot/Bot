    ...
    async setNewDaily(prize, isReminder, option, transferUser) {
        ...
        if (transferUser) {
            ...
            const transaction = {
                ...
                data: `${this.emojis.gain} Ganhou ${prize.money} Safiras transferidas do ${prize.day}º dia do *daily* de ${this.user.username}.`
            }

            socket?.send({
                type: "transactions",
                transactionsData: { value: prize.money, userId: transferUser.id, transaction }
            })

            data.$push = { Transactions: { $each: [transaction], $position: 0 } }
        ...
        if (prize.day > 0) {
            ...
            socket?.send({
                type: "transactions",
                transactionsData: { value: prize.money, userId: this.user.id, transaction }
            })
